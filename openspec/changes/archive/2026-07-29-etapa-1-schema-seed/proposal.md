...
- [x] `pnpm db:migrate` creates all 7 tables with enums and FKs on a clean DB
- [x] `pnpm db:seed` populates 5 mines × 12 months and 5 suppliers idempotently
- [x] `pnpm test` passes; generators tested RED→GREEN including all 3 variance events
- [x] Seed output byte-identical across repeated runs (deterministic)
