...
- [ ] Each adapter unit-tested (success, missing key, HTTP error) with mocked `fetch`.
- [ ] `FallbackChain` falls through on failure and throws when all providers fail.
- [ ] `POST /api/llm/complete` returns `{ text, provider, model }` and maps errors via `llm.*` keys.
- [ ] `createLlmChain()` guarded by `import "server-only"`; no client bundle exposure.
- [ ] es/en parity test passes with new `llm.*` keys.
